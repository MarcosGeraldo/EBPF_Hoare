import EBPFSpec.H_Ver_Cond_Gen_Sound
...
theorem incrMem_correct (v : ℕ) :
  ...
  · -- Demonstra que a pré-condição do usuário implica a pré-condição mais fraca do bloco
    intro s h1
    simp [blockWP]
    simp [incrMem]
    rcases s
    · simp
      exact h1
  · -- A tripla central validada pelo blockWP_sound corrigido (com Q explícito e tática by intro)
    exact blockWP_sound incrMem incrMem 0 (fun s => s.[0] = some (v + 1)) (by intro k; simp)
  · -- A pós-condição é trivialmente implicada
    intro s hs
    exact hs
